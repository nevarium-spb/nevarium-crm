...
2. **Домен CRM: `crm-nevarium.ru`** — куплен в Ru-Center (nic.ru), оплачен до 27.07.2027.
   Сейчас делегирован на статусную страницу Ru-Center (`statuspage1/2.nic.ru`) —
   при развёртывании переключить DNS на IP сервера Timeweb (см. шаг 5).
   Отдельный домен, а не поддомен любого из сайтов: CRM обслуживает оба бизнеса,
   и селить её под доменом одного из них было бы неверно.
...
5. **DNS**: в Ru-Center у `crm-nevarium.ru` переключить DNS-серверы со статусной страницы
   на A-запись → IP вашего VPS (или CNAME, если Cloud Apps даёт свой адрес).
